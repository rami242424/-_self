...
if (Boolean('hello')) {
    console.log('실행되었음!') // (실행되었음이 출력되는가?) -> 실행되었음!
}
...
Boolean(1) // true
Boolean(-1) // true
Boolean(100) // true ->숫자는 0뺴고 전부 true

Boolean(null) // false
Boolean(undefined) // false
Boolean(NaN) // false
// !!null // 실무에서 true, false를 구분할 때 사용
// !!null -> false, !!'hello' -> true
...
x.pop() // 2
x.pop() // 1
//-> js: x가 가리키고 있는 값은 변한적이 없는데 true, false로 바뀌는 것은 옳지 않다. (제 생각이 아닙니다.)

Boolean([1, 2, 3]) // true  // [1,2,3]: 배열 -> true
Boolean([]) // true
Boolean({ 'one': 1 }) // true
Boolean({}) // true
